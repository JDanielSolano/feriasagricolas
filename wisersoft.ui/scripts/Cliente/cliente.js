...
function mostrarGoogleMaps(val1, val2) {
    ...
    google.maps.event.addListener(markup, 'drag', function () {
        $("#Cl_Latitude").val(markup.getPosition().lat());
        $("#Cl_Longitud").val(markup.getPosition().lng());
...
function BuscarCantones() {
    ...
    if (Pro_id != "") {
        ...
        $("#Cantones_cnt_id").append($('<option></option>').val("").html("Seleccione un canton:"));
        $("#Cl_Distrito").html("");
        $("#Cl_Distrito").append($('<option></option>').val("").html("Seleccione un distrito:"));
    }
}
    ...
    if (Cant_id != "") {
        $.ajax(
        {
            ...
            success: function (selectDistritos) {
                $("#Cl_Distrito").html(""); // clear before appending new list
                $("#Cl_Distrito").append($('<option></option>').val("").html("Seleccione un distrito:"));
                $.each(selectDistritos, function (i, distrito) {
                    $("#Cl_Distrito").append($('<option></option>').val(distrito.Value).html(distrito.Text));
                });
            },
            error: function (xhr, status, error) {
                alert(xhr.responseText);
